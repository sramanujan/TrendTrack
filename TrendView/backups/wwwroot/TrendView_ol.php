...
if(isset($_POST['pw'])&&isset($_POST['db']))
{
	$result = mysql_query("select * from db_info where `DataBase` = '".$_POST['db']."'") or die(mysql_error());
	...
	if($row[5]==$_POST['pw'])
	{
	    ...
	    echo "</tr></table>";
	    echo "</div>";
		echo "<form name='form1' action='TrendView.php' method='post'>";
	    $curdat=getdate();
	    ...
	    if(isset($_POST['yearsel']))
	        ...
	        $ych=$curdat['year'];
	    $result = mysql_query("select distinct year(time) from ".$_POST['db']) or die(mysql_error());
	    ...
	    {
	        if(isset($_POST['monthsel']))
	            $mch=$_POST['monthsel'];
	        else
	            $mch=$curdat['month'];
	        echo "<option>Select</option>";
	        $result = mysql_query("select distinct monthname(time) from `".$_POST['db']."` where year(time) = ".$ych."") or die(mysql_error());
			$flag=0;
	        while($row = mysql_fetch_array($result))
	        {
	            echo "<option";
	    ...
	    {
	        $result = mysql_query("select distinct day(time) from `".$_POST['db']."` where year(time) = ".$ych." and monthname(time) = '".$mch."'") or die(mysql_error());
	        echo "<option>Select</option>";
			$flag=0;
	        while($row = mysql_fetch_array($result))
	        {
	    ...
	    if(isset($_POST['daysel']))
	    {
	        if(strcmp($_POST['daysel'],"Select")!=0)
	            $q3=" AND DAY(TIME) = ".$_POST['daysel'];
	        else
	        {
	            $q3=" ";
	            $t=3;
	    ...
	    if(isset($_POST['monthsel']))
	    {
	        if(strcmp($_POST['monthsel'],"Select")!=0)
	            $q2=" AND MONTHNAME(TIME) = '".$_POST['monthsel']."'";
	        else
	        {
	            $q2=" ";
	            $t=2;
	    ...
	    if(isset($_POST['yearsel']))
	    {
	        if(strcmp($_POST['yearsel'],"Select")!=0)
	            $q1=" AND YEAR(TIME) = ".$_POST['yearsel'];
	        else
	        {
	            $q1=" ";
	            $t=1;
	    ...
	    case 0:
	    {
	        ...
	        $sub=mysql_query("select distinct hour(time) from `".$_POST['db']."` where 1".$q1.$q2.$q3." order by hour(time)") or die(mysql_error());
	    }
	    break;
	    case 1:
	    {
	        $query=" AND YEAR(TIME) = ";
	        $sub=mysql_query("select distinct year(time) from `".$_POST['db']."` where 1 order by year(time)") or die(mysql_error());
	    }
	    break;
	    case 2:
	    {
	        $query=$q1." AND MONTH(TIME) = ";
	        $sub=mysql_query("select distinct month(time) from `".$_POST['db']."` where 1".$q1." order by month(time)") or die(mysql_error());
	    }
	    break;
	    case 3:
	    {
	        $query=$q1.$q2." AND DAY(TIME) = ";
	        $sub=mysql_query("select distinct day(time) from `".$_POST['db']."` where 1".$q1.$q2." order by day(time)") or die(mysql_error());
	    }
	    break;
	    }
		...
		if(isset($_POST['sex']))
		{
			if(isset($_POST['age']))
			{
				...
				while($cnt==1)
				{
					$sum = 0;
					$sum2 = 0;
					$result = mysql_query("select 0+sex,0+age,sum(walkin),sum(purchase) from `".$_POST['db']."` where 1".$query.$row2[0]." group by sex,age") or die(mysql_error());
					echo "['".$row2[0]."',";
					$array[1]=0;
					$array[2]=0;
					$array[3]=0;
			...
			else
			{
				...
				while($cnt==1)
				{
					$sum = 0;
					$sum2 = 0;
					$result = mysql_query("select 0+sex,sum(walkin),sum(purchase) from `".$_POST['db']."` where 1".$query.$row2[0]." group by sex") or die(mysql_error());
					echo "['".$row2[0]."',";
					$array[1]=0;
					$array[2]=0;
					while($row = mysql_fetch_array($result))
		...
		else
		{
			if(isset($_POST['age']))
			{
				...
				while($cnt==1)
				{
					$sum = 0;
					$sum2 = 0;
					$result = mysql_query("select 0+age,sum(walkin),sum(purchase) from `".$_POST['db']."` where 1".$query.$row2[0]." group by age") or die(mysql_error());
					echo "['".$row2[0]."',";
					$array[1]=0;
					$array[2]=0;
					$array[3]=0;
			...
			else
			{
				...
				while($cnt==1)
				{
					$sum = 0;
					$sum2 = 0;
					$result = mysql_query("select sum(walkin),sum(purchase) from `".$_POST['db']."` where 1".$query.$row2[0]) or die(mysql_error());
					echo "['".$row2[0]."',";
					$array=0;
					while($row = mysql_fetch_array($result))
					{
	    ...
	    echo "}";
	    echo "</script>";
		echo "<button type='button' onClick=window.open('export.php?ych=".$ych."&mch=".$mch."&dch=".$dch."&pw=".$_POST['pw']."&db=".$_POST['db'];
		if(isset($_POST['sex']))
			echo "&sex=Yes";
		if(isset($_POST['age']))
			echo "&age=Yes";
	...
	{
		echo "<form name='form1' action='TrendView.php' method='post'><br \><br \><br \><br \><br \>";
	    echo "<br \><br \><br \><br \><br \><br \><table align='center'>";
		echo "<tr><td  align='center'><h1>Welcome to TrendView!</h1><br \></tr></td>";
	    echo "<tr><td  align='center'>DataBase :  <input type='text' name='db' value='rt_orr'/></tr></td>";
	    echo "<tr><td  align='center'>Password :  <input type='password' name='pw' value='' /></tr></td>";
	    echo "<tr><td  align='center'><input type='submit' value='Go!!' /></tr></td>";
		echo "<tr><td  align='center'>Invalid DataBase or Password... Please try again!</tr></td></table>";
	    echo "</form>";
	}
}
else
{
    ...
    echo "<tr><td  align='center'>DataBase :  <input type='text' name='db' value='rt_orr'/></tr></td>";
    echo "<tr><td  align='center'>Password :  <input type='password' name='pw' value='' /></tr></td>";
    echo "<tr><td  align='center'><input type='submit' value='Go!!' /></tr></td></table>";
    echo "</form>";
}
