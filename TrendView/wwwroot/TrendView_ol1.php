...
if(isset($_COOKIE['trendtrack']))
{
	if(isset($_GET['logout']))
	{
		setcookie("trendtrack", "", time()-60);
		header('Location: TrendView.php');
		echo "
			<html>
	...
	else 
	{
		...
		{
			$result = mysql_query("select `store_info`,`Database` from `db_info` where `login` = '".$_COOKIE["trendtrack"]."'") or die(mysql_error());
...
{
	$result = mysql_query("select `pwd` from `company_info` where `login` = '".$_POST['login']."'") or die(mysql_error());
	$row = mysql_fetch_array($result);
	if($_POST['pwd']==$row[0])
	{
		setcookie("trendtrack", $_POST['login'], time()+604800) ;
	}
	header('Location: TrendView.php');
	echo "
	<html>
...
else
{
	...
	echo "<tr><td  align='right'>Login :  <input type='text' name='login' value=''/></tr></td>";
	echo "<tr><td  align='right'>Password :  <input type='password' name='pwd' value='' /></tr></td>";
	echo "<tr><td  align='right'><input type='submit' value='   Go!!   ' /></tr></td></table>";
	echo "</form>";
}
